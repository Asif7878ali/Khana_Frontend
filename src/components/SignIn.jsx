...
const SignIn = () => {
  ...
  const emptyValues = {
    ...
    const regSchema = Yup.object({
      ...
      email : Yup.string().email('Enter a Valid E-mail').required('E-mail is Required').test('is-gmail', 'Email domain must be @gmail.com', value =>{
        if(value){
          return value.endsWith('@gmail.com')
        }
        return false
     }),
      password : Yup.string().matches(/^(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.{8,})/, 'Password must meet the criteria')
     ...
     <div className="w-full bg-grey-lightest mt-2">
      <div className="container mx-auto py-8">
        ...
        <form className="space-y-4 md:space-y-6" action="" onSubmit={handleSubmit}>
          <div className="py-4 px-8 text-black text-xl border-b border-grey-lighter">Register for a Account</div>
          <div className="py-4 px-8">
            <div className="flex mb-4">
                   {/* FirstName */}
              <div className="w-1/2 mr-1">
                <label className="block text-grey-darker text-sm font-bold mb-2" htmlFor="first_name">First Name</label>
                <input className="appearance-none border rounded w-full py-2 px-3 text-grey-darker" name='first_name'
                  id="first_name" type="text" placeholder="Your First Name" autoComplete=""
              ...
              <div className="w-1/2 ml-1">
                ...
                <input className="appearance-none border rounded w-full py-2 px-3 text-grey-darker" name='last_name'
                  id="last_name" type="text" placeholder="Your Last Name" autoComplete=""
            ...
            <div className="mb-4">
              ...
              <input className="appearance-none border rounded w-full py-2 px-3 text-grey-darker" autoComplete=""
                id="email" type="email" name='email' placeholder="Your E-mail Address"
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleChange} onBlur={handleBlur} name='password' value={values.password} autoComplete=''/>
                <div className="icon_button absolute right-4 top-14" onClick={visibilityPassword}>
                  {showPassword ? (<RiEyeFill className="h-6 font-extralight" />) : (
            ...
            {errors.password && touched.password ? <p className="text-red-600">{errors.password}</p> : null}
              ...
              <p className="text-xs mt-1 text-red-600">At least 8 characters</p>
            </div>

                 {/* ConfirmPassword */}
            <div className="mb-4">
              <label className="block text-grey-darker text-sm font-bold mb-2" htmlFor="password">Confirm Password</label>
              <div className="relative">
                <input
                  className="input block border border-gray-300 focus:border-pitch-black  py-3 px-3 w-full focus:outline-none "
                  type= {confirmShowPassword ? 'text' : 'password'} placeholder='Confirm Password'
                  onChange={handleChange} onBlur={handleBlur} name='confirm_password' value={values.confirm_password} autoComplete=''/>
                <div className="icon_button absolute right-4 top-14" onClick={visibilityConfirmPassword}>
                  {confirmShowPassword ? (<RiEyeFill className="h-6 font-extralight" />) : (
                                   <RiEyeOffFill className="h-6 font-extralight" /> )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mt-8">
              ...
              <p className="text-center my-4"> I already have an account
              <Link to="/login"> 
              <span className="text-black hover:underline cursor-pointer text-sm font-bold">Login</span>
              </Link>
